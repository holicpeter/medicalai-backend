...
class HealthDataExtractor:
    ...
    def _init_patterns(self) -> Dict:
        return {
            ...
            'glucose': [
                r'(?:glukóza|glukoza|glykémia|glykemia|glucose|GLU|Glu)[\s:]*(\d+[.,]?\d*)',
            ],
            'hba1c': [
                r'HbA1c[\s:]*(\d+[.,]?\d*)',
            ],
            'cholesterol': [
                r'(?:cholesterol|CHOL|Chol)[\s:]*(\d+[.,]?\d*)',
            ],
            'ldl': [r'LDL[\s:]*(\d+[.,]?\d*)'],
            'hdl': [r'HDL[\s:]*(\d+[.,]?\d*)'],
            'triglycerides': [
                r'(?:triglyceridy|TG|TAG)[\s:]*(\d+[.,]?\d*)',
            ],
            'creatinine': [
                r'(?:kreatinin|creatinine|KREAT|Krea)[\s:]*(\d+[.,]?\d*)',
            ],
            'alt': [r'(?:ALT|ALAT|SGPT)[\s:]*(\d+[.,]?\d*)'],
            'ast': [r'(?:AST|ASAT|SGOT)[\s:]*(\d+[.,]?\d*)'],
            'hemoglobin': [r'(?:hemoglobin|Hb|HGB)[\s:]*(\d+[.,]?\d*)'],
            'leukocytes': [r'(?:leukocyty|WBC|Le)[\s:]*(\d+[.,]?\d*)'],
            'platelets': [r'(?:trombocyty|PLT|Tr)[\s:]*(\d+[.,]?\d*)'],
            'bmi': [r'BMI[\s:]*(\d+[.,]?\d*)'],
            'weight': [r'(?:hmotnosť|váha|weight)[\s:]*(\d+[.,]?\d*)\s*kg'],
            'date': [
                r'(\d{1,2})[./](\d{1,2})[./](\d{4})',
                r'(\d{4})[-/](\d{1,2})[-/](\d{1,2})',
            ],
